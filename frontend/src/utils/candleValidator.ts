...
export function validateCandles(candles: any[]): ValidationResult {
    ...
    const requiredFields = ['symbol', 'interval', 'openTime', 'closeTime', 'open', 'high', 'low', 'close'];

    for (const field of requiredFields) {
        if (!(field in sample)) errors.push(`Missing required field: ${field}`);
    }

    if (typeof sample.openTime !== 'number') errors.push('openTime must be number');
    if (typeof sample.open !== 'number') errors.push('open must be number');
    if (typeof sample.high !== 'number') errors.push('high must be number');
    if (typeof sample.low !== 'number') errors.push('low must be number');
    if (typeof sample.close !== 'number') errors.push('close must be number');
